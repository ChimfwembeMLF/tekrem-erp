...
export default function ProjectKanban({ auth, project, milestones }: KanbanProps) {
  ...
  const [columns, setColumns] = useState<KanbanColumn[]>([
    ...
    },
    {
      id: 'blocked',
      title: 'Blocked',
      milestones: milestones.filter(m => m.status === 'blocked')
    },
    {
      id: 'completed',
      title: 'Completed',
  ...
  const handleDragEnd = (result: DropResult) => {
    ...
    router.put(route('projects.milestones.update', [project.id, milestone.id]), {
      status: newStatus
    });
  };
    ...
    >
      ...
      <div className="py-6">
        <DragDropContext onDragEnd={handleDragEnd}>
          <div className="flex flex-row overflow-x-auto gap-6 w-full w-12xl">
            {columns.map((column) => (
