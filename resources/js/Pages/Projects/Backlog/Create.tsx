...
export default function CreateBacklog({
  ...
  const { data, setData, post, processing, errors } = useForm({
    ...
    type: 'product' as 'product' | 'sprint',
    story_points: '',
    status: 'new' as 'new' | 'ready' | 'in_progress' | 'done' | 'removed',
    sprint_id: '',
    epic_id: '',
    assigned_to: '',
    order: '0',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const submitData = {
      ...
      sprint_id:
        data.sprint_id && data.sprint_id !== 'no' ? data.sprint_id : null,
      epic_id: data.epic_id && data.epic_id !== 'no' ? data.epic_id : null,
      assigned_to:
        data.assigned_to && data.assigned_to !== 'no' ? data.assigned_to : null,
      order: data.order ? parseInt(data.order) : 0,
    };

    post(route('agile.backlog.store', project.id), { data: submitData });
  };

  return (
    <AppLayout
      title={`Create Backlog Item - ${project.name}`}
      renderHeader={() => (
        <div>
    ...
    >
      <Head title={`Create Backlog Item - ${project.name}`} />
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  />
                  {errors.story_points && (
                    <p className="text-sm text-red-500">{errors.story_points}</p>
                  )}
                </div>
              </div>
              ...
              {sprints.length > 0 && (
                <div className="space-y-2">
                  ...
                  {errors.sprint_id && (
                    <p className="text-sm text-red-500">{errors.sprint_id}</p>
                  )}
                </div>
              )}
