...
export default function Index({ invoices, filters, statuses, billables }: Props) {
  ...
  const [selectedZraStatus, setSelectedZraStatus] = useState(filters.zra_status || 'all');
  ...
  const handleSearch = () => {
    router.get(route('finance.invoices.index'), {
      ...
      zra_status: selectedZraStatus === 'all' ? '' : selectedZraStatus,
            ...
            <CardContent>
              <div className="flex flex-col sm:flex-row justify-between mb-4 gap-2">
                ...
                <div className="flex gap-2">
                  ...
                  <Select value={selectedZraStatus} onValueChange={setSelectedZraStatus}>
                    <SelectTrigger className="w-40">
                      <SelectValue placeholder={t('finance.zra.all_statuses', 'All ZRA Status')} />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="all">{t('finance.zra.all_statuses', 'All ZRA Status')}</SelectItem>
                      <SelectItem value="not_submitted">{t('finance.zra.status.not_submitted', 'Not Submitted')}</SelectItem>
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              ...
                              {invoice.zra_status === 'not_submitted' && !invoice.is_zra_locked && (
                                <DropdownMenuItem onClick={() => router.post(route('finance.zra.submit', invoice.id))}>
                                  <Shield className="mr-2 h-4 w-4" />
                                  {t('finance.zra.submit', 'Submit to ZRA')}
                                </DropdownMenuItem>
                              )}
                              {(invoice.zra_status === 'rejected' || invoice.zra_status === 'cancelled') && !invoice.is_zra_locked && (
                                <DropdownMenuItem onClick={() => router.post(route('finance.zra.resubmit', invoice.id))}>
