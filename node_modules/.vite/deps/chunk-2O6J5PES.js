...
var F = class {
  ...
  visit(e, { method: i = "get", data: r = {}, replace: s = false, preserveScroll: l = false, preserveState: h = false, only: g = [], except: m = [], headers: c = {}, errorBag: o = "", forceFormData: v = false, onCancelToken: L = () => {
    ...
    let R = { url: S, method: i, data: r, replace: s, preserveScroll: l, preserveState: h, only: g, except: m, headers: c, errorBag: o, forceFormData: v, queryStringArrayFormat: A, cancelled: false, completed: false, interrupted: false };
    ...
    this.activeVisit = { ...R, onCancelToken: L, onBefore: d, onStart: p, onProgress: x, onFinish: y, onCancel: ne, onSuccess: U, onError: G, queryStringArrayFormat: A, cancelToken: new AbortController() }, L({ cancel: () => {
      this.activeVisit && this.cancelVisit(this.activeVisit, { cancelled: true });
    } }), X(R), p(R);
    let j = !!(g.length || m.length);
    axios_default({ method: i, url: E(S).href, data: i === "get" ? {} : r, params: i === "get" ? r : {}, signal: this.activeVisit.cancelToken.signal, headers: { ...c, Accept: "text/html, application/xhtml+xml", "X-Requested-With": "XMLHttpRequest", "X-Inertia": true, ...j ? { "X-Inertia-Partial-Component": this.page.component } : {}, ...g.length ? { "X-Inertia-Partial-Data": g.join(",") } : {}, ...m.length ? { "X-Inertia-Partial-Except": m.join(",") } : {}, ...o && o.length ? { "X-Inertia-Error-Bag": o } : {}, ...this.page.version ? { "X-Inertia-Version": this.page.version } : {} }, onUploadProgress: (n) => {
      r instanceof FormData && (n.percentage = n.progress ? Math.round(n.progress * 100) : 0, K(n), x(n));
    } }).then((n) => {
      var _a;
      ...
      let w = S, V = b(a.url);
      return w.hash && !V.hash && E(w).href === V.href && (V.hash = w.hash, a.url = V.href), this.setPage(a, { visitId: M, replace: s, preserveScroll: l, preserveState: h });
    }).then(() => {
...
nprogress/nprogress.js:
   ...
   * @license MIT *)
*/
//# sourceMappingURL=chunk-2O6J5PES.js.map
